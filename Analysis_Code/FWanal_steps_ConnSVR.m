...
% Which svr result? 
% 1 = right, info; 2 = right, non-info
% 3 = left, info; 4 = left, non-info
file_n = 4;

% -------------------------------------------------------------------------
%% File names

filename{1} = [study_name '_GROUPRES_NSBJ28_win10_steps10_av1_st3_SVR_LIBSVM_DCGR_I.mat'];
filename{2} = [study_name '_GROUPRES_NSBJ28_win10_steps10_av1_st3_SVR_LIBSVM_DCGR_NI.mat'];
filename{3} = [study_name '_GROUPRES_NSBJ28_win10_steps10_av1_st3_SVR_LIBSVM_DCGL_I.mat'];
filename{4} = [study_name '_GROUPRES_NSBJ28_win10_steps10_av1_st3_SVR_LIBSVM_DCGL_NI.mat'];
...
step_scale(2,1:end) = 187.5 : (ANALYSIS.step_width_ms*5) : 787;

% Change labels for plots and save names
step_scale_label = ANALYSIS.xaxis_scale;
step_scale_label(2,1:end) = (187.5 : (ANALYSIS.step_width_ms*5) : 787) - ANALYSIS.pointzero;
...
FW.steps_for_testing = 6; %corr - no info, L
FW.disp_steps = FW.steps_for_testing;


% Add steps to plot & fig names
step_plotadd = ['(' num2str(step_scale_label(2,FW.steps_for_testing(1))) '-' ...
    num2str(step_scale_label(2,FW.steps_for_testing(end))) ' ms)'];
step_figadd = ['_' num2str(step_scale_label(2,FW.steps_for_testing(1))) '-' ...
    num2str(step_scale_label(2,FW.steps_for_testing(end))) 'ms'];
...
% Names to save plots
plotname{1} = [study_name ': Info Cue, Right' step_plotadd];
plotname{2} = [study_name ': Non-Info, Right' step_plotadd];
plotname{3} = [study_name ': Info Cue, Left' step_plotadd];
plotname{4} = [study_name ': Non-Info, Left' step_plotadd];

% Names to save figs
figname{1} = [study_name step_figadd '_SVR_R_I.fig'];
figname{2} = [study_name step_figadd '_SVR_R_NI.fig'];
figname{3} = [study_name step_figadd '_SVR_L_I.fig'];
figname{4} = [study_name step_figadd '_SVR_L_NI.fig'];

clear step_plotadd step_figadd
...
FWanal_ConnDecoding_plot(ANALYSIS,FW,saveFig,figname,plotname,file_n,chanlocs);
