...
% output_dir = [bdir '\Decode_Results\'];
output_dir = [bdir '\Decode_Results_v3\']; %for classification
...
nchannels = 31; % Number of channels
...
pointzero = 1000; %for lowFreq_amp
...
if strcmpi(study_name,'lowFreq_amp')
    % For lowFreq_amp (20 = 50 steps)
%     window_width_ms = 20; % Width of sliding analysis window in ms
%     step_width_ms = 20; % Step size with which sliding analysis window is moved through the trial
    window_width_ms = 13; % Width of sliding analysis window in ms
    step_width_ms = 13; % Step size with which sliding analysis window is moved through the trial
else
   % For all other features (15 = 50 steps)
    window_width_ms = 15; % Width of sliding analysis window in ms
    step_width_ms = 15; % Step size with which sliding analysis window is moved through the trial 
end
%---------------------------------------------------------------------------
...
for dcg_set = 1
    ...
    analyse_decoding_erp_mine(ANALYSIS);
