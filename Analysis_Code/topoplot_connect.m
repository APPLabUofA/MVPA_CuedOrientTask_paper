...
BACKCOLOR = [1 1 1];  % white (EEGLAB standard sucks!)
...
for kk = 1:numChanPairs
    if ~displayStruct.connectStrength(kk) == 0
        plot3(y(displayStruct.chanPairs(kk, :)), x(displayStruct.chanPairs(kk, :)), [ELECTRODE_HEIGHT, ELECTRODE_HEIGHT], 'LineWidth', 1.5, 'Color', cM(cmapPos(kk), :));
    end
end
% praneeth - ends
