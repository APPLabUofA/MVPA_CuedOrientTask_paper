...
function [pval, t_orig, tmx_ptile, corrected_p, corrected_h, seed_state, est_alpha] = mxt_perm1_Phipson_Smyth(...
    data,n_perm,alph,tail,verblevel,seed_state,freq_domain)
...
for t = 1:n_pts
    for c = 1:n_chan
        ...
        elseif tail == 1 %upper tailed
            pval(c,t) = mean(mxt>=t_orig(c,t));
        elseif tail == -1 %lower tailed
            pval(c,t) = mean(mxt<=t_orig(c,t));
        end
    end
    clear c
...
% test results
corrected_h = zeros(n_chan,n_pts); % Preallocate
corrected_p = zeros(n_chan,n_pts); % Preallocate
...
for t = 1:n_pts
    for c = 1:n_chan
        ...
        if tail == 0 % If using two-tailed testing

            b = sum(mxt(:) >= abs(t_orig(c,t)));
            p_t = (b + 1) / (n_perm + 1); % Calculate conservative version of p-value as in Phipson & Smyth, 2010
            p_t = p_t .* 2; % Doubling of p-value for two-tailed testing (essentially Bonferroni correction for two tests)
            corrected_p(c,t) = p_t;

        elseif tail == 1 % One-tailed testing for positive differences

            b = sum(mxt(:) >= t_orig(c,t));
            p_t = (b + 1) / (n_perm + 1); % Calculate conservative version of p-value as in Phipson & Smyth, 2010
            corrected_p(c,t) = p_t;

        elseif tail == -1 % One-tailed testing for negative differences

            b = sum(mxt(:) >= t_orig(c,t) .* -1);
            p_t = (b + 1) / (n_perm + 1); % Calculate conservative version of p-value as in Phipson & Smyth, 2010
            corrected_p(c,t) = p_t;
...
corrected_h(corrected_p < alph) = 1;
