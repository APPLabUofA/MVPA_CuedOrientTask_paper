function [M] = modindex_Tort(Phase, Amp, s_rate)
...
totalTime = length(Amp)/s_rate;

% phase bins for slower frequency
...
%     end    
    ...
    for k = 1:N
        ...
        index = find(wrapTo360(circ_rad2ang(Phase)) >= PhaseBins(k) & wrapTo360(circ_rad2ang(Phase)) < PhaseBins(k)+((360)/N));

        % amplitudes that are within the designated phase bins 
        Bin(k).amp = mean(Amp(index));
    end
end
